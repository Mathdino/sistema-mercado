...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log('Validating product data:', { name, description, price, image, categoryId, unit, stock });
    if (!name || !description || price === undefined || !image || !categoryId || !unit || stock === undefined) {
      console.log('Validation failed:', { name, description, price, image, categoryId, unit, stock });
      return NextResponse.json({ 
        error: "missing_required_fields", 
        details: { name, description, price, image, categoryId, unit, stock }
      }, { status: 400 })
    }
    ...
    if (isNaN(parsedPrice) || parsedPrice <= 0) {
      return NextResponse.json({ error: "invalid_price", price }, { status: 400 })
    }
    ...
    if (isNaN(parsedStock) || parsedStock < 0) {
      return NextResponse.json({ error: "invalid_stock", stock }, { status: 400 })
    }
