...
export default function AdminOrdersPage() {
        ...
        if (response.ok) {
          ...
          const transformedOrders: Order[] = data.orders.map((order: any) => {
            ...
            return {
              ...
              paymentMethod:
                order.paymentMethod.toLowerCase() as Order["paymentMethod"],
              deliveryAddress: {
  ...
  const handleStatusChange = async (
    ...
    try {
      console.log(
        `Updating order ${orderId} to status ${newStatus.toUpperCase()}`
      );
  ...
  const generateOrderPDF = async (order: Order) => {
    try {
      ...
      const fullAddress = `Endereço: ${order.deliveryAddress.street}, ${
                      ...
                      <div className="flex items-start justify-between">
                        <div className="space-y-1">
                          ...
                          <CardTitle className="text-base">
                            Pedido #{shortenOrderId(order.id)}
                          </CardTitle>
        ...
        >
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              ...
              <DialogTitle>
                Detalhes do Pedido #{shortenOrderId(selectedOrder?.id || "")}
              </DialogTitle>
              <DialogDescription>
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-sm font-medium">
                    {selectedOrder.user?.name || "Não disponível"}
                  </p>
                  <p className="text-sm">
                    {selectedOrder.user?.phone || "Telefone não disponível"}
                  </p>
                </div>
