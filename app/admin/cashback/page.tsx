...
import { CalendarIcon, Menu, X, ChevronLeft, ChevronRight, LayoutDashboard, ShoppingBag, Package, Users, DollarSign, Building, Truck, LogOut, Tag } from "lucide-react";
...
export default function AdminCashbackPage() {
  ...
  const [globalCashback, setGlobalCashback] = useState<GlobalCashback | null>(null);
  ...
  const navItems = [
    ...
    { icon: Tag, label: "Cashback", path: "/admin/cashback" }, // Usando Tag pois é o ícone mais próximo de cashback
  ];
  ...
  const [isUpdatingGlobalCashback, setIsUpdatingGlobalCashback] = useState(false);
  ...
  const updateGlobalCashback = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({
          title: "Sucesso",
          description: "Configurações de cashback global atualizadas com sucesso",
        });
      } else {
        const errorData = await response.json();
        console.error("Error response from API:", errorData);
        throw new Error(`Failed to update global cashback: ${errorData.error || response.status}`);
      }
    } catch (error) {
  ...
  const deleteCoupon = async (id: string) => {
    try {
      ...
      if (response.ok) {
        // Update the coupon to be inactive instead of removing it from the list
  ...
  return (
    ...
    <div className="flex min-h-screen bg-background">
          ...
          <nav className={cn("flex-1 space-y-1 p-4", !sidebarOpen && "px-2")}>
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  <Icon
                    className={cn("h-5 w-5", sidebarOpen && "mr-3")}
                  />
      ...
      {/* Conteúdo principal */}
      <main className={cn("flex-1 p-6 md:p-8", sidebarOpen ? "md:ml-64" : "md:ml-16")}>
        <div className="p-6 space-y-6">
          <div>
            <h1 className="text-3xl font-bold">Cashback e Promoções</h1>
            <p className="text-muted-foreground">Gerencie cashbacks recorrentes e cupons de desconto</p>
          </div>

      <Tabs defaultValue="global" className="space-y-6">
        <TabsList>
          <TabsTrigger value="global">Cashback Recorrente</TabsTrigger>
          <TabsTrigger value="cashbacks">Cashbacks Individuais</TabsTrigger>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center justify-between">
                <Label htmlFor="global-cashback-toggle">Ativar Cashback Recorrente</Label>
                <Switch
              ...
              <div className="space-y-2">
                <Label htmlFor="global-cashback-percentage">Porcentagem de Cashback (%)</Label>
                <Input
                  ...
                  onChange={(e) => setGlobalCashbackPercentage(parseFloat(e.target.value) || 0)}
                  disabled={!isGlobalCashbackActive}
                />
                <p className="text-sm text-muted-foreground">
                  Porcentagem de cashback que será aplicada a todos os clientes quando o pedido anterior for finalizado
                </p>
              </div>
              ...
              <Button 
                ...
                className="w-full"
              >
                {isUpdatingGlobalCashback ? "Atualizando..." : "Salvar Configurações"}
              </Button>
            </CardContent>
          </Card>
        </TabsContent>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <Select 
                      ...
                      onValueChange={(value) => setNewCashback({...newCashback, userId: value})}
                    >
                      <SelectTrigger>
                  ...
                  <div>
                    <Label htmlFor="cashback-percentage">Porcentagem (%)</Label>
                    <Input
                      ...
                      onChange={(e) => setNewCashback({...newCashback, percentage: parseFloat(e.target.value) || 0})}
                    />
                  </div>
                </div>
                <DialogFooter>
              ...
              <Card key={cashback.id}>
                  ...
                  <CardTitle className="flex justify-between items-start">
                    <div>
                      ...
                      <div className="text-sm font-normal text-muted-foreground">
                        {cashback.percentage}% - {cashback.isActive ? "Ativo" : "Inativo"}
                      </div>
                    </div>
                    <Button
                      variant="outline"
                ...
                <CardContent>
                  <div className="text-sm space-y-1">
                    <div><span className="font-medium">Criado:</span> {new Date(cashback.createdAt).toLocaleString('pt-BR')}</div>
                    {cashback.previousOrder && (
                      <div>
                        <span className="font-medium">Pedido anterior:</span> R$ {cashback.previousOrder.totalAmount.toFixed(2)}
                      </div>
                    )}
                  </div>
                </CardContent>
          ...
          <div className="flex justify-between items-center">
            ...
            <Dialog open={isAddCouponOpen} onOpenChange={setIsAddCouponOpen}>
              ...
              <DialogContent className="max-w-md">
                ...
                <div className="space-y-4">
                  <div>
                    <Label className="mb-2" htmlFor="coupon-code">Código</Label>
                    <Input
                      id="coupon-code"
                  ...
                  <div>
                    <Label className="mb-2" htmlFor="coupon-type">Tipo</Label>
                    <Select 
                      value={newCoupon.type} 
                      onValueChange={(value) => setNewCoupon({...newCoupon, type: value as "FIXED" | "PERCENTAGE"})}
                    >
                      <SelectTrigger>
                  ...
                  <div>
                    <Label className="mb-2" htmlFor="coupon-discount">Desconto</Label>
                    <Input
                      id="coupon-discount"
                      type="number"
                      min="0"
                  ...
                  <div>
                    <Label className="mb-2" htmlFor="coupon-max-uses">Máximo de Usos (0 = ilimitado)</Label>
                    <Input
                      id="coupon-max-uses"
                      type="number"
                      min="0"
                  ...
                  <div>
                    <Label className="mb-2" htmlFor="coupon-expires-at">Data de Expiração</Label>
                    <Popover>
                      <PopoverTrigger asChild>
                        ...
                        >
                          ...
                          {newCoupon.expiresAt ? format(new Date(newCoupon.expiresAt), "PPP", { locale: ptBR }) : <span>Selecione uma data</span>}
                        </Button>
                      </PopoverTrigger>
                      <PopoverContent className="w-auto p-0">
                        <Calendar
                          ...
                          onSelect={(date) => setNewCoupon({...newCoupon, expiresAt: date ? date.toISOString() : ""})}
                          initialFocus
                        />
                      </PopoverContent>
                    </Popover>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {coupons.map(coupon => (
              <Card key={coupon.id}>
                    ...
                    <div>
                      ...
                      <div className="text-sm font-normal text-muted-foreground">
                        {coupon.type === "PERCENTAGE" 
                          ? `${coupon.discount}%` 
                          : formatCurrency(coupon.discount)} - {coupon.isActive ? "Ativo" : "Inativo"}
                      </div>
                    </div>
                    <Button
                      variant="outline"
                ...
                <CardContent>
                  <div className="text-sm space-y-1">
                    <div><span className="font-medium">Tipo:</span> {coupon.type === "PERCENTAGE" ? "Porcentagem" : "Valor Fixo"}</div>
                    <div><span className="font-medium">Expira:</span> {coupon.expiresAt ? new Date(coupon.expiresAt).toLocaleDateString('pt-BR') : "Nunca"}</div>
                    <div><span className="font-medium">Usos:</span> {coupon.usedCount}{coupon.maxUses ? `/${coupon.maxUses}` : ''}</div>
                    <div><span className="font-medium">Criado:</span> {new Date(coupon.createdAt).toLocaleString('pt-BR')}</div>
                  </div>
                </CardContent>
              </Card>
            ))}
